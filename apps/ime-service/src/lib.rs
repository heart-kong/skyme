...
impl ImeService {
    ...
    pub fn initialize(shared_dir: &str, user_dir: &str, dist_name: &str) -> RimeResult<()> {
        ...
        if let (Some(pk), Some(gc), Some(fc), Some(gx), Some(fx)) = (engine.rime_process_key_fn(), engine.rime_get_commit_fn(), engine.rime_free_commit_fn(), engine.rime_get_context_fn(), engine.rime_free_context_fn()) {
            skyme_ime_core::com::text_service::set_rime_fns(pk, gc, fc, gx, fx);
        }
